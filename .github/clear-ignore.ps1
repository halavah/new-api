    ...
    if ($fileCount -gt 0) {
        ...
        if ($confirm -eq "y" -or $confirm -eq "Y") {
            ...
            if ($successCount -gt 0) {
                Write-Host "🔄 自动提交并推送..." -ForegroundColor Yellow
                git add -A
                git commit -m "chore: remove ignored files from Git tracking" 2>$null
                if ($?) { Write-Host "✅ 已 commit" -ForegroundColor Green }
                else { Write-Host "ℹ️  无变更需要 commit" -ForegroundColor Gray }
                git push origin HEAD 2>$null
                if ($?) { Write-Host "✅ 已推送" -ForegroundColor Green }
                else { Write-Host "⚠️  推送失败（请手动 git push）" -ForegroundColor Yellow }
            }
