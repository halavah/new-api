...
if (-not (Test-Path $gitDir)) {
    Write-Host ""
    Write-Host "[FAIL] 错误: 当前目录不是 Git 仓库" -ForegroundColor Red
    Write-Host ""
    Read-Host "按回车键退出"
    exit 1
}
...
if (-not $lastCommit) {
    Write-Host "[FAIL] 错误: 无法获取提交信息，仓库可能没有任何提交" -ForegroundColor Red
    Write-Host ""
    Read-Host "按回车键退出"
    exit 1
}
...
# 显示最后一次提交信息
Write-Host "[NOTE] 最后一次提交信息:" -ForegroundColor Cyan
Write-Host ""
...
if ($unpushedCommits) {
    Write-Host "[WARN]  警告: 本地有未推送的提交" -ForegroundColor Yellow
...
Write-Host "[STAT] 当前状态:" -ForegroundColor Cyan
if ($statusOutput) {
...
# 警告提示
Write-Host "[WARN]  操作说明:" -ForegroundColor Yellow
...
if ($confirmation -eq "y" -or $confirmation -eq "Y" -or $confirmation -eq "yes") {
    Write-Host ""
    Write-Host "[SYNC] 正在撤销提交..." -ForegroundColor Cyan
    ...
    try {
        ...
        Write-Host ""
        Write-Host "[OK] 成功撤销最后一次提交" -ForegroundColor Green
        Write-Host ""
        Write-Host "[NOTE] 撤销后的状态:" -ForegroundColor Cyan
        ...
        Write-Host ""
        Write-Host "[TIP] 提示: 使用 'git commit' 重新提交这些更改" -ForegroundColor Cyan
        Write-Host ""

    } catch {
        Write-Host ""
        Write-Host "[FAIL] 错误: 撤销提交失败" -ForegroundColor Red
        Write-Host "   $_" -ForegroundColor Red
        Write-Host ""
    }
} else {
    Write-Host ""
    Write-Host "[FAIL] 操作已取消" -ForegroundColor Red
    Write-Host ""
}
